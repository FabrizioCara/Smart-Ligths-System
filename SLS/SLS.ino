...
void loop() {
  ...
  if (millis() > Bot_lastTime + Bot_delay) { // in questa sezione si risponde ai messaggi in ingresso, prima di iniziare bisogna inserire la parola START!
    ...
    if (startControlWithTelegram == 1 && bot.message[0][0].toInt() == 1) {
      getMessageAndAnswer();
      Bot_lastTime = millis();
      Serial.println("ricevi messaggio position");
    }
  }
...
void lightController() {
  ...
  if (brightnessValue == "High") {
    ...
    detectPresence(); // aggiorna il vettore delle presenze
    Serial.println(String(presence[0]) + " " + String(presence[1]) + " " + String(presence[2]));
    if (oneIsInArray() == 1) { // accendo le luci in cui c'è una presenza al 100%, mentre le altre all'80%
      for (int i = 0; i < 3; i++) {
        if (presence[i] == 1) {
          ...
          Serial.println("Lamp " + String(i) + " 100%");
        }
        else {
          analogWrite(transistorList[i], 204); //80%
          Serial.println("Lamp " + String(i) + " 80%");
        }
      }
      Soff = 0;
      S1 = 0;
      ...
      for (int i = 0; i < 3; i++) {
        analogWrite(transistorList[i], 0);
        Serial.println("Lamp " + String(i) + " 0%");
      }
      Soff = 1; // il sistema è spento
      S1 = 0;
      S2 = 0;
...
String detectBrightness() {
  ...
  if (brightness < 700) {                                                         // <--------- Da modificare
    return "Low";
  }
  else if (brightness > 700 && brightness < 850) {                                // <--------- Da modificare
    return "Medium";
  }
  else if (brightness > 850) {                                                    // <--------- Da modificare
    return "High";
  }
}
...
void sendValuesToThingSpeak() {
  ...
  if (client.connect(server, 80)) {
    ...
    if (sendPavgcount == PavgLenght) { //ogni min
      Paverage();
      postStr += "&field4=";
      postStr += String(lastPavg);
      sendPavgcount = 0;
      Serial.print("Last Pavg inviato:");
      Serial.println(lastPavg);
    }
